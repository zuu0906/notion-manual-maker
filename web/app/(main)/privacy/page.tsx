...
export const metadata: Metadata = {
  title: 'プライバシーポリシー',
  description: 'Chrome Manual Makerのプライバシーポリシー。収集する情報・利用目的・データの取り扱いについて説明します。',
    ...
    <section className="max-w-3xl mx-auto px-4 py-20">
      ...
      <div className="flex flex-col gap-10 text-sm text-gray-700 leading-relaxed">
        ...
        <div>
          ...
          <p>
            本プライバシーポリシーは、Chrome Manual Maker（以下「本サービス」）の運営者（お問い合わせ先：support@s-tasklog.com）が、
            ユーザーの個人情報をどのように取り扱うかを定めたものです。
          </p>
        </div>
        ...
        <div>
          ...
          <p>
            記録されたスクリーンショットはSupabaseのストレージを経由してNotionページに保存されます。
            アップロード完了後はNotionページから参照される公開URLとして扱われます。
          </p>
          <p className="mt-2">
            個人情報の自動ぼかし機能（メールアドレス・電話番号・クレジットカード番号・パスワード欄など）は、
            お使いのデバイス上のみで処理され、サーバーには送信されません。
          </p>
        </div>
        ...
        <div>
          ...
          <p>
            ユーザーデータはアカウント削除まで保持されます。
            アカウントを削除した場合、メールアドレス・利用統計・ストレージ上のスクリーンショットは速やかに削除されます。
            Notionに保存されたページはNotionアカウント上に残りますので、必要に応じてNotion側で削除してください。
          </p>
        </div>
        ...
        <div>
          ...
          <p>
            本サービスはセッション管理のためにCookieおよびlocalStorageを使用します。
