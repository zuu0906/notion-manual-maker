(() => {
  ...
  hint.textContent = '記録したい箇所をクリック　拡張機能アイコンで停止';
  ...
  function getFormNote(el) {
    ...
    const SKIP_TAGS = new Set(['A', 'BUTTON', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'NAV', 'HEADER', 'FOOTER']);
  ...
  function promptTextInput(el, clickX, clickY) {
    return new Promise((resolve) => {
      const hint = el.getAttribute('placeholder') || el.getAttribute('aria-label') || el.getAttribute('name') || 'この入力欄';
      const card = createCard(`「${hint}」に入力する内容`);
      ...
      input.placeholder = '入力する内容を記録...';

      const btnRow = document.createElement('div');
      btnRow.style.cssText = 'display:flex;gap:6px;justify-content:flex-end;';

      const cancelBtn = createBtn('スキップ', '#f0f0f0', '#333');
  ...
  function showPasswordConfirm() {
    return new Promise((resolve) => {
      const card = createCard('パスワード欄への入力を記録');

      const msg = document.createElement('p');
      msg.style.cssText = 'font-size:12px;color:#555;margin-bottom:10px;line-height:1.5;';
      msg.textContent = '入力内容は記録されません。「パスワードを入力」としてステップに記録します。';
