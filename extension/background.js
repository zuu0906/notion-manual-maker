...
async function saveToNotion(notionPageId, title, steps) {
  ...
  await incrementScreenshotCount(pendingClicks.length);
  ...
  chrome.action.setBadgeText({ text: '' });
  await saveSession();
  return { success: true, url: `https://notion.so/${notionPage.id.replace(/-/g, '')}` };
}
...
async function createNotionPage(token, title) {
  ...
  return res.ok ? res.json() : { error: 'Notionページ作成失敗' };
}
...
async function appendNotionBlocks(token, pageId, children) {
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message ?? `Notion blocks append failed: HTTP ${res.status}`);
  }
}
